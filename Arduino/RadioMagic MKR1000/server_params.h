 ...
 * Server IP address is 10.0.0.2:8081\n
 * All the commands are independent and correspond to a preset feature of the synth.
 * If the user - for example - set the wave or PWM mode on one of the three synths
...
//! Server IP address
inline int IP(int x) { int ip[] = {10, 0, 0, 2}; return ip[x]; }
//! Acces Point IP gateway
inline int GATEWAY(int x) { int ip[] = {10, 0, 0, 1}; return ip[x]; }
//! Acces Point IP DNS
inline int DNS(int x) { int ip[] = {10, 0, 0, 1}; return ip[x]; }
//! Acces Point subnet
